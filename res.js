      ...
      const unsubscribe = onSnapshot(slotDataRef, (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (data.slotId !== undefined) {
            fetchedSlots.set(data.slotId, data.status); // Use the slotId as is from Firestore
          }
        });
        ...
        setSlotSets((currentSlotSets) => {
          return currentSlotSets.map((floor) => ({
            ...
            slots: floor.slots.map((slot, index) => {
              // Correctly reconstruct the slotId for comparison with fetchedSlots
              const slotId = floor.title === 'General Parking' ? `General-${index}` : `${floor.title}-${index}`;
              const isOccupied = fetchedSlots.get(slotId) === 'Occupied';
              console.log(`Slot ${slotId} is set to ${isOccupied ? 'occupied' : 'not occupied'}`);
              return {
                ...slot,
                occupied: isOccupied,
              };
